...
bool JedecDRAMSystem::WillAcceptTransaction(uint64_t hex_addr, bool is_write,
  ...
  return ctrls_[channel]->WillAcceptTransaction(hex_addr, is_write, is_prio);
}
...
// Record trace - Record address trace for debugging or other purposes
  ...
  address_trace_ << std::hex << hex_addr << std::dec << " "
                 << (is_write ? "WRITE " : "READ ") << " "
                 << (is_prio ? "PRIO " : "NPRIO") << clk_ << std::endl;
#endif

  int channel = GetChannel(hex_addr);
  bool ok = ctrls_[channel]->WillAcceptTransaction(hex_addr, is_write, is_prio);

  assert(ok);
  if (ok) {
    Transaction trans = Transaction(hex_addr, is_write, is_prio);
...
bool IdealDRAMSystem::AddTransaction(uint64_t hex_addr, bool is_write,
                                     bool is_prio) {
  auto trans = Transaction(hex_addr, is_write, is_prio);
  trans.added_cycle = clk_;
  if (is_prio) {
    prio_infinite_buffer_q_.push_back(trans);
  } else {
...
void IdealDRAMSystem::ClockTick() {
  if (prio_infinite_buffer_q_.size() == 0) {
    // Prio queue empty
    ...
    // Prio transaction exists
    for (auto prio_trans_it = prio_infinite_buffer_q_.begin();
         prio_trans_it != prio_infinite_buffer_q_.end();) {
      if (clk_ - prio_trans_it->added_cycle >= static_cast<uint64_t>(latency_)) {
        if (prio_trans_it->is_write) {
          write_callback_(prio_trans_it->addr);
        } else {
          read_callback_(prio_trans_it->addr);
        }
        prio_trans_it = prio_infinite_buffer_q_.erase(prio_trans_it++);
      }
      if (prio_trans_it != prio_infinite_buffer_q_.end()) {
        ++prio_trans_it;
      }
    }
  }
