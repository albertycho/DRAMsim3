...
Controller::Controller(int channel, const Config &config, const Timing &timing)
  ...
  if (is_unified_queue_) {
    ...
    prio_unified_queue_.reserve(config_.trans_queue_size);
  } else {
    read_queue_.reserve(config_.trans_queue_size);
    write_buffer_.reserve(config_.trans_queue_size);
    prio_read_queue_.reserve(config_.trans_queue_size);
    prio_write_buffer_.reserve(config_.trans_queue_size);
  }
...
std::pair<uint64_t, int> Controller::ReturnDoneTrans(uint64_t clk) {
  // All cmds are returned on same clk_
  // So we can just use one return queue, since instructions are all
  // retired together
  ...
  while (it != return_queue_.end()) {
    if (clk >= it->complete_cycle) {
      ...
      if (it->is_prio) {
        simple_stats_.Increment("prio_trans_done");
      }
...
void Controller::ClockTick() {
  ...
  if (cmd.IsValid()) {
    ...
    // Only in HBM, which we aren't testing
  ...
  // power updates pt 1
  // Update stat counters for self refresh & all_bank_idle_cycles or
  // rank_active_cycles
    ...
    for (auto i = 0; i < config_.ranks; i++) {
      if (channel_state_.IsRankSelfRefreshing(i)) {
        ...
        if (!cmd_queue_.rank_q_empty[i]) {
          auto addr = Address();
          addr.rank = i;
          // TODO: See if this can be made to be prio
          auto cmd = Command(CommandType::SREF_EXIT, addr, -1, false);
          cmd = channel_state_.GetReadyCommand(cmd, clk_);
          if (cmd.IsValid()) {
            // TODO: Check does this ever happen? SREF_EXIT is not a case in
            // GetReadyCommand at all. Thus does prio matter here?
        ...
        // Enter sref mode if all ranks are empty
        if (cmd_queue_.rank_q_empty[i] &&
...
bool Controller::WillAcceptTransaction(uint64_t hex_addr, bool is_write,
                                       bool is_prio) const {
  if (is_unified_queue_) {
    return prio_unified_queue_.size() + unified_queue_.size() <
           unified_queue_.capacity();
  } else if (!is_write) {
    return prio_read_queue_.size() + read_queue_.size() <
           read_queue_.capacity();
  } else {
    return prio_write_buffer_.size() + write_buffer_.size() <
           write_buffer_.capacity();
  }
}

bool Controller::AddTransaction(Transaction trans) {
  // TODO: Add prio handling
  ...
  if (trans.is_write) {
    if (pending_wr_q_.count(trans.addr) == 0) { // can not merge writes
      ...
      if (trans.is_prio) {
        if (is_unified_queue_) {
          prio_unified_queue_.push_back(trans);
        } else {
          prio_write_buffer_.push_back(trans);
        }
      } else {
        if (is_unified_queue_) {
    ...
    if (pending_rd_q_.count(trans.addr) == 1) {
      if (trans.is_prio) {
        if (is_unified_queue_) {
          ...
          prio_read_queue_.push_back(trans);
        }
      } else {
        if (is_unified_queue_) {
          unified_queue_.push_back(trans);
...
void Controller::ScheduleTransaction() {
  ...
  if (write_draining_ == 0 && !is_unified_queue_) {
    ...
    if ((write_buffer_.size() + prio_write_buffer_.size() >=
         write_buffer_.capacity()) ||
        (write_buffer_.size() + prio_write_buffer_.size() > 8 &&
         cmd_queue_.QueueEmpty())) {
      write_draining_ = write_buffer_.size() + prio_write_buffer_.size();
    }
  }

  // 1st drain prio
  // TODO: Check if command queue can be reordered
  std::vector<Transaction> &prio_queue = is_unified_queue_ ? prio_unified_queue_
                                         : write_draining_ > 0
                                             ? prio_write_buffer_
                                             : prio_read_queue_;
  for (auto it = prio_queue.begin(); it != prio_queue.end(); it++) {
    ...
    if (cmd_queue_.WillAcceptCommand(cmd.Rank(), cmd.Bankgroup(), cmd.Bank(),
      ...
      if (!is_unified_queue_ && cmd.IsWrite()) {
        ...
        // Prio still uses same
        if (pending_rd_q_.count(it->addr) > 0) {
          // If there is a read pending on this address, don't write to it
          write_draining_ = 0;
          return;
        }
        write_draining_ -= 1;
  ...
  // Online 1 transaction should be scheduled per cycle
...
Command Controller::TransToCommand(const Transaction &trans) {
  ...
  return Command(cmd_type, addr, trans.addr, trans.is_prio);
}
...
void Controller::UpdateCommandStats(const Command &cmd) {
  ...
  if (cmd.IsPrio()) {
    simple_stats_.Increment("num_prio_cmds");
  }
}
