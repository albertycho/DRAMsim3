...
MemorySystem::MemorySystem(const std::string &config_file,
  ...
  /*
   * dram_system_ = 
   *   new IdealDRAMSystem(*config_, output_dir, read_callback, write_callback);
   */
  if (config_->IsHMC()) {
...
int MemorySystem::GetTransQueueSize() const {
  return config_->trans_queue_size;
}

int MemorySystem::GetPrioQueueSize() const { return config_->prio_queue_size; }
...
bool MemorySystem::WillAcceptTransaction(uint64_t hex_addr,
  ...
  return dram_system_->WillAcceptTransaction(hex_addr, is_write, is_prio);
}

bool MemorySystem::AddTransaction(uint64_t hex_addr, bool is_write, bool is_prio) {
  return dram_system_->AddTransaction(hex_addr, is_write, is_prio);
}
