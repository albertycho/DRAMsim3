...
CommandQueue::CommandQueue(int channel_id, const Config &config,
  ...
  prio_queues_.reserve(num_queues_);
  for (int i = 0; i < num_queues_; i++) {
    auto cmd_queue = std::vector<Command>();
    cmd_queue.reserve(config_.cmd_queue_size);
    queues_.push_back(cmd_queue);
    // Prio
    auto prio_cmd_queue = std::vector<Command>();
    prio_cmd_queue.reserve(config_.cmd_queue_size);
    prio_queues_.push_back(prio_cmd_queue);
  }
}

Command CommandQueue::GetCommandToIssue() {
  // First check prio queues
  for (int i = 0; i < num_queues_; i++) {
    auto &queue = GetNextQueue(true);
...
bool CommandQueue::WillAcceptCommand(int rank, int bankgroup, int bank,
  ...
  return prio_queues_[q_idx].size() + queues_[q_idx].size() < queue_size_;
}
...
bool CommandQueue::AddCommand(Command cmd) {
  auto &def_queue = GetQueue(cmd.Rank(), cmd.Bankgroup(), cmd.Bank(), false);
  auto &prio_queue = GetQueue(cmd.Rank(), cmd.Bankgroup(), cmd.Bank(), true);
  if (def_queue.size() + prio_queue.size() < queue_size_) {
    auto &queue = GetQueue(cmd.Rank(), cmd.Bankgroup(), cmd.Bank(), cmd.IsPrio());
    queue.push_back(cmd);
...
CMDQueue &CommandQueue::GetNextQueue(bool is_prio) {
  if (is_prio) {
    prio_queue_idx_++;
    if (prio_queue_idx_ == num_queues_) {
      prio_queue_idx_ = 0;
    }
    return prio_queues_[prio_queue_idx_];
  } else {
    queue_idx_++;
