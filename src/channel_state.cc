  ...
  if (cmd.IsRankCMD()) {
    ...
    for (auto j = 0; j < config_.bankgroups; j++) {
      for (auto k = 0; k < config_.banks_per_group; k++) {
        ...
        // Must be PRECHARGE, as the precharge ocurrs before refresh in open
        // state if the open row is not the row that this cmd accesses
        if (ready_cmd.cmd_type != cmd.cmd_type) { // likely PRECHARGE
          // Send precharge cmd for this rank to prepare for a refresh
    ...
    // Per rank staggered, so will send cmd when all banks are ready
    // All bank ready
